...
//    double time =exponential(1 / lamda);
    EV << "next generate time: " << simTime() + time << endl;
...
// Note: the only function to modify is "check_channel_busy".
...
//    }
    ...
    int csma_type = this->par("csma_type");//0:non 1:1 2:p
    ...
    {
//        send_num_same_time--;
        if (csma_type == 0 || csma_type == 1 || (csma_type ==2 && num < p) ){
            bubble("checking channel: channel is not busy, send RTS");
            this->scheduleAt(simTime(), send_RTS_broadcast_event);
            if (csma_type ==2) EV << num << ":" << p << endl;
            EV <<"checking channel: channel is not busy, send RTS" << endl;
            backoff_stage=2;//reset the back-off stage
        } else {
            bubble("checking channel: channel is not busy, but not send RTS");
            backoff_counter=(int)uniform(1,cw_min*pow(2,backoff_stage));
            this->scheduleAt(simTime(), random_backoff_event);
            if (csma_type ==2) EV << num << ":" << p << endl;
            EV <<"checking channel: channel is not busy, but not send RTS" << endl;
            backoff_stage=2;
        }
    }
    else //if channel is busy, then goto random back-off
    {
        ...
        if(csma_type == 0) {
            backoff_counter=(int)uniform(1,cw_min*pow(2,backoff_stage));
            this->scheduleAt(simTime(), random_backoff_event);
            EV << "random_backoff_event" << endl;
        } else {
            EV << "check_channel_busy_event time: " << simTime() + pow(10,-4) << endl;
            bubble("slot check");
            this->scheduleAt(simTime() + pow(10,-4), check_channel_busy_event);
        }
    }
}
